...
theme.render_app_header(
    "State Comparison",
    subtitle="Compare disparity profiles across two or more states/UTs.",
...
selected_states = st.sidebar.multiselect("States / UTs (2 or more)", options=all_states, default=default_states)
...
if scoped_df.empty:
    st.warning("No records match selection.")
    st.stop()
...
with chart_cols[0]:
    ...
    fig_bar.update_layout(title="Average Underserved Probability", yaxis_tickformat=".0%", showlegend=False)
...
with chart_cols[1]:
    ...
    fig_radar.update_layout(
        title="Relative Disparity Profile (normalized)",
...
if st.button("Generate Narrated Comparison", type="primary"):
    shap_explainer, _ = ds.get_shap_explainer(artifacts)
    worst_per_state = (
